 ...
 * @since  1.0.0
 */
class WHDI_WXR_Importer {

    /**
     * Instance of WHDI_WXR_Importer
     *
     * @since  1.0.0
     * @var WHDI_WXR_Importer
     */
    private static $_instance = null;

    /**
     * Instantiate WHDI_WXR_Importer
     *
     * @since  1.0.0
     * @return (Object) WHDI_WXR_Importer.
     */
    ...
    private function __construct() {
        ...
        require_once WHDI_DIR . '/includes/importers/wxr-importer/class-logger.php';
        require_once WHDI_DIR . '/includes/importers/wxr-importer/class-wp-importer-logger-serversentevents.php';
        require_once WHDI_DIR . '/includes/importers/wxr-importer/class-wxr-importer.php';
        require_once WHDI_DIR . '/includes/importers/wxr-importer/class-wxr-import-info.php';

        add_filter( 'upload_mimes', array( $this, 'custom_upload_mimes' ) );
        add_action( 'wp_ajax_whdi-wxr-import', array( $this, 'sse_import' ) );
    ...
    function track_post( $post_id ) {
        WHDI_Importer_Log::add( 'Inserted - Post ' . $post_id . ' - ' . get_post_type( $post_id ) . ' - ' . get_the_title( $post_id ) );
        update_post_meta( $post_id, '_whdi_sites_imported_post', true );
    }
    ...
    function track_term( $term_id ) {
        ...
        if ( $term ) {
            WHDI_Importer_Log::add( 'Inserted - Term ' . $term_id . ' - ' . json_encode( $term ) );
        }
        update_term_meta( $term_id, '_whdi_sites_imported_term', true );
    }
    ...
    public function get_xml_data( $path ) {
        ...
        return array(
            ...
            'url'     => $url,
            'strings' => array(
                'complete' => __( 'Import complete!', 'wphobby-demo-import' ),
            ),
        );
    }
